...
export abstract class IBudgetStrategy {
  private props: IBudgetStrategyProps;
  ...
  constructor(construct: Stack, props: IBudgetStrategyProps) {
    ...
    if (props.defaultTopic) {
      this._defaultTopic = new sns.Topic(
        this.stack,
        `${props.defaultTopic}-topic`,
        {
  ...
  protected abstract createDailyBudgets(
    dailyLimit: number,
    subscribers: Array<budgets.CfnBudget.SubscriberProperty>
  ): void;
  protected abstract createMonthlyBudgets(
    monthlyLimit: number,
    subscribers: Array<budgets.CfnBudget.SubscriberProperty>
  ): void;
  protected abstract createQuarterlyBudgets(
    quarterlyLimit: number,
    subscribers: Array<budgets.CfnBudget.SubscriberProperty>
  ): void;
  protected abstract createYearlyBudgets(
    yearlyLimit: number,
    subscribers: Array<budgets.CfnBudget.SubscriberProperty>
  ): void;
  ...
  public createBudgets(): void {
    ...
    this.createQuarterlyBudgets(
      this.quarterlyLimit,
      this.createAllSubscribers()
    );
  ...
  private createAllSubscribers(): Array<budgets.CfnBudget.SubscriberProperty> {
    ...
    if (snsSubscriber) {
      return [...this.createEmailSubscribers(), snsSubscriber];
    } else {
  ...
  private createEmailSubscribers(): Array<budgets.CfnBudget.SubscriberProperty> {
    return (
      this.props.subscribers?.map((email) => {
        return {
